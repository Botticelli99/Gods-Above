const GA_ROUTES = {
  accueil: { title: "Gods Above — Accueil", file: "pages/accueil.html" },
  magie: { title: "Magie & Systèmes — Gods Above", file: "pages/magie.html" },
...
function rewriteInternalLinks(container) {
  container.querySelectorAll("a[href]").forEach(link => {
    ...
    if (GA_ALIASES[cleanFile]) {
      link.setAttribute("href", "#/" + GA_ALIASES[cleanFile] + (anchor ? "#" + anchor : ""));
...
function cleanupPageGlobals() {
  // Stop intervals/timeouts created by previous pages only if they registered themselves later.
  // Kept intentionally simple for compatibility.
}

function exportPageFunctionsToWindow(scriptText) {
  const names = [...scriptText.matchAll(/\bfunction\s+([A-Za-z_$][\w$]*)\s*\(/g)].map(match => match[1]);
...
async function loadRoute(routeName) {
  if (!app) {
    console.error("Impossible de trouver #ga-app dans index.html");
    return;
  }
